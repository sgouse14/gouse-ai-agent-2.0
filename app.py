...
app = FastAPI(title="Gouse AI Architecture Agent", version="2.6.0")
agent = GouseAIAgent(OpenAIClient()); memory = FileMemory(); projects = ProjectStore(); architecture = ArchitectureAnalyzer()
report_agent = ArchitectureReportAgent(agent); intelligence = ProjectIntelligenceEngine(report_agent)
vision = ArchitectureVisionAnalyzer(); render_prompts = RenderPromptBuilder()
...
class MaterialInput(BaseModel):
    name: str; category: str; unit: str = ""; quantity: float | None = Field(default=None, ge=0); rate: float | None = Field(default=None, ge=0); notes: str = ""
...
@app.get("/api/health")
def health(): return {"status":"ok","agent":"Gouse AI Architecture"}
...
def analyze_project(project_id: str, request: ProjectIntelligenceRequest):
    ...
    result = intelligence.analyze(project, UPLOAD_DIR, request.focus)
    projects.add_analysis(project_id, result["title"], result["analysis"])
    return result
@app.post("/api/chat")
...
async def upload_document(file: UploadFile = File(...)):
    ...
    safe_name=f"{uuid4().hex}_{Path(file.filename).name}"; destination=UPLOAD_DIR/safe_name; destination.write_bytes(await file.read()); summary=extract_text(destination)
