...
app = FastAPI(title="Gouse AI Architecture Agent", version="2.5.0")
...
def chat(request: ChatRequest):
    ...
    memory.add("user", request.message); response = agent.run(request.message); memory.add("assistant", response.text)
...
async def upload_document(file: UploadFile = File(...)):
    ...
    safe_name = f"{uuid4().hex}_{Path(file.filename).name}"; destination = UPLOAD_DIR / safe_name
    destination.write_bytes(await file.read()); summary = extract_text(destination)
...
def analyze_image(request: VisionAnalysisRequest):
    ...
    response = agent.run(vision.build_analysis_prompt(VisionRequest(str(path), request.focus)))
...
def create_render_prompt(request: RenderRequest):
    if not request.description.strip(): raise HTTPException(400, "A project description is required")
